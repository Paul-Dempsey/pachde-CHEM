...
bool Sostenuto2Ui::connected() {
    if (!my_module) return false;
    if (!chem_host) return false;
    return true;
}

Sostenuto2Ui::Sostenuto2Ui(Sostenuto2Module *module) :
    my_module(module)
{
    setModule(module);
    initThemeEngine();
...
void Sostenuto2Ui::onConnectHost(IChemHost* host)
{
    chem_host = host;
    if (chem_host) {
...
void Sostenuto2Ui::onConnectionChange(ChemDevice device, std::shared_ptr<MidiDeviceConnection> connection)
{
...
void Sostenuto2Ui::step()
{
    Base::step();
    if (!my_module) return;
}

void Sostenuto2Ui::appendContextMenu(Menu *menu)
{
    if (!module) return;
