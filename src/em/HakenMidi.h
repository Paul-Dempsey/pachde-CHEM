// Haken Audio Midi Protocol by Lippold Haken, (C) Copyright 1999-2025.
...
//	  32	Overview		
//	  61	Status Bytes    Definitions for Continuum		(all)
//   173	SSL_ch1_ch2     List of all cc on ch1/ch2       (ch1/ch2)
//	 198	SSL_ch16        List of all cc on ch16		    (ch16)
//   228    SSL_streams		List of all stream names		(ch16)
//   237    SSL_Form_Poke	List of all id in s_Form_Poke	(ch16)
//   256    SSL_Mat_Poke	List of all id in s_Mat_Poke	(ch16)
//   281	SSL_ccTask		List of values for ccTask		(ch16)
//	 309	Preset Select   Preset Load and Store			(ch2/ch16)
//	 420	Note cc         For Encoding Surface Activity	(ch2..ch16)
//	 449	Macro cc        Macros i..vi and 14' m7..m90	(ch1 and ch2)
//	 492	Global cc       Absolute ch1 and Relative ch2	(ch1 and ch2)
//	 537	Setup cc        General Configuration			(ch16)
//	 988	ccStream        Select a stream					(ch16 "sData" follow)
//	1067	s_Form_Poke     Formula Configuration stream	(ch16 "sData" pairs: id,value)
//	1273	s_Mat_Poke      Matrix  Configuration stream	(ch16 "sData" pairs: id,value)
//	1589	s_Kinet_Poke    Kinetic Properties stream		(ch16 "sData" pairs: id,value)
//	1644	s_BiqSin_Poke   BiqSin  Properties stream		(ch16 "sData" pairs: id,value)
//	1707	s_Conv_Poke     Convolution Config stream 		(ch16 "sData" pairs: id,value)
//	1751	Thick Continuum config strip locations
...
enum {
	...
	nnLow22 = 52,	// nominal lowest note number (without transpose) on slim422
	nnLow22fsh = 54,	// lowest f# on a slim22
	nnHigh22 = 73,	// nominal highest note number (without transpose) on slim22
	nnHigh22c = 72,	// highest c on a slim22
...
#define inv_max14  0.00006151574 // 1/max14 = 1/(127<<7); ccFracXYZ/ccFracIM48/ccFracM49M90 = low7'
	zero14 = 0x2000,// 0.0 value for 14' -1..1 controllers
#define inv_zero14 0.00012207031 // 1/zero14 = 1/(64<<7); ccFracXYZ/ccFracIM48/ccFracM49M90 = low7'
...
#define SSL_ch16				  /* see sections below for definitions of these cc */					  \
			   ...
			   /* ch 16 cc83  */   "[EqTilt] [EqFrq] [EqMix] [FracPed] Frac [MSpL] [MSpH] [ThrDrv] "	  \
			   /* ch 16 cc91  */   "[AtkCut] [RatMkp] [CoThMix] (94) [Reci5] [Reci6] [FracPedEx] "		  \
...
#define SSL_Mat_Poke	       /* see s_Mat_Poke section below for definitions of these id */		  \
			...
			/* stream 20 id 35  */ "Program Routing (37) (38) Poly BendRange "							  \
...
#define SSL_ccTask 			   /* see ccTask below for definitions of these values */				  \
			...
			/* ccTask value 36  */ "36nop 37nop 38nop sysToMidi "	                                      \
			...
			/* ccTask value 107 */ "Demo1 Demo2 Demo3 Demo4 Demo5 Demo6 Demo7 Demo8 "	                  \
...
// ------------------------------------- Macros i..vi and 14' m7..m90 -------------------------------------
	...
	// 
	// NB: Preset files have macros i..vi on ch16 as aliases for i..vi on ch1.
	...
	// ------------------------------ Global cc: Absolute ch1 or Relative ch2  ------------------------------
	// For all controllers assignable to pedals (including 7' and 14' controllers), see routine pedVal7().
	// NB: Saved preset files have these on ch16, but those will be moved to ch1.
	// NB: Some of these (e.g. ccOctShift) are rx ch1 but tx ch16 (for Editor).
	...
	// one of two loopback detect mechanisms, see also ccMini_LB
	...
	hw_s22 = 7, hw_s46, hw_s70,	// hardw field: slim22, slim46, slim70						 9.08
	...
	//  After startData, two download memory bytes are encoded per dData00/01/10/11 message.
	...
	userToMidi = 32,	// rx: request sSys, and analysis names, and uPreset names
	//      with their cats, and current preset definition		 6.19 10.41
	...
	//				= 36,	//     retired full midi transmission rate
	...
	anlysToMidi = 50,	// rx: request analysis names (subset of userToMidi)		10.43
...
s_AnlysName = 27,	// Ascii name of Additive Analysis 				(max  32 sData)
...
#define				inv_maxBendR 0.0104166667
...
#define				isSmooth(mf)  ((0b001001011 >> (mf)) & 1) // 6,3,1,0 are smooth
	...
	MLegacyCh1out = 6,	// legacy note output starting on ch1						10.64
